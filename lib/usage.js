...
export function summarize(overridePrices) {
  ...
  for (const e of list) {
    ...
    const bk = e.book || '(未标书)'
    byBook[bk] = byBook[bk] || { calls: 0, ok: 0, fail: 0, prompt: 0, completion: 0, cost: 0 }
    byBook[bk].calls++
    if (e.ok) byBook[bk].ok++; else byBook[bk].fail++
    byBook[bk].prompt += e.promptTokens || 0
    byBook[bk].completion += e.completionTokens || 0
    byBook[bk].cost += costOf(e, overridePrices)
  }
  ...
  return { total, byModel, byDate, byEffort, byBook, entries: list.length, mode: isCny ? "cny" : "usd" }
}
...
export function report(overridePrices) {
  ...
  const books = Object.keys(s.byBook)
  if (books.length) {
    lines.push('')
    lines.push('按书:')
    for (const bk of books) {
      const v = s.byBook[bk]
      lines.push('- ' + bk + ': ' + v.calls + ' 次（成功 ' + v.ok + ' / 失败 ' + v.fail + '）, 输入 ' + v.prompt + ', 输出 ' + v.completion + ', ' + fee(v.cost))
    }
  }

...
export class NovelEngine {
  ...
  async generate(opts) {
    ...
    if (!res.ok) {
      ...
      recordUsage({ model, ok: false, error: message, durationMs: Date.now() - t0 });
      throw new Error(message);
    }
    const data = await res.json();
    const u = data && data.usage ? data.usage : {};
    recordUsage({ model, ok: true, promptTokens: u.prompt_tokens || 0, completionTokens: u.completion_tokens || 0, cacheHit: u.prompt_cache_hit_tokens || 0, cacheMiss: u.prompt_cache_miss_tokens || 0, durationMs: Date.now() - t0 });

...
export class NovelEngine {
  ...
  _conceptIdx() {
    if (!this._conceptConn) this._conceptConn = new ConceptIndex(path.join(this.root, ".writing-concepts.json"), this);
  ...
  async generate(opts) {
    ...
    // V4 思考模式：官方默认开启（effort=high），思考 token 计入 max_tokens 且不支持 temperature。
    // low/默认 = 关闭思考（快、省、temperature 生效，适合创意正文）；high/max = 开启思考 + 对应档位。
    // 旧模型名（deepseek-chat/reasoner）由模型名自决，不传 thinking 参数。
    const isV4 = /v4-(flash|pro)/.test(model);
    const think = isV4 && (effort === "high" || effort === "max");
    const body = {
      model: model,
      messages: messages,
      stream: false,
    ...
    if (isV4) body.thinking = { type: think ? "enabled" : "disabled" };
    if (think) body.reasoning_effort = effort;
    if (!think && opts.temperature !== undefined) body.temperature = opts.temperature;
    ...
    // 空输出偶发（思考模式偶发吃满额度/服务端抖动）：自动重试一次
    if (!content.trim() && !opts.noRetry) {
      const res2 = await doFetch(AbortSignal.timeout(timeoutMs));
      if (res2.ok) {
        const data2 = await res2.json();
        const u2 = data2 && data2.usage ? data2.usage : {};
        recordUsage({ model, effort, ok: true, promptTokens: u2.prompt_tokens || 0, completionTokens: u2.completion_tokens || 0, cacheHit: u2.prompt_cache_hit_tokens || 0, cacheMiss: u2.prompt_cache_miss_tokens || 0, durationMs: Date.now() - t0 });
        const c2 = data2 && data2.choices && data2.choices[0] && data2.choices[0].message
          ? data2.choices[0].message.content : "";
        if (typeof c2 === "string" && c2.trim()) content = c2;
      }
    }
    return content;
  }
  ...
  async generateMany(tasks, opts) {
    ...
    // 前缀缓存预热：批量任务共享同一前缀，先串行跑首任务让服务端建立缓存，
    // 其余任务并发时自动命中（命中价约为未命中的 1/50~1/120）。noWarmup 可跳过。
    if (tasks.length > 1 && !opts.noWarmup) {
      await runOne(0);
      let next = 1;
      const worker = async () => {
        while (true) {
      ...
      const wc = Math.min(limit, tasks.length - 1);
  ...
  async removeChapter(chapterId) {
    ...
    const rest = chapters.filter(function (x) { return x.id !== chapterId; });
    manifest.chapters = rest;
    if (manifest.lastChapter === chapterId) manifest.lastChapter = rest.length ? rest[rest.length - 1].id : "";
    await this.writeManifest(manifest);
    await this.rebuildCatalog();
    return { removed: 1, id: chapterId };
  }

  async renameChapter(chapterId, title, renameBody) {
    const manifest = await this.readManifest();
    const chapters = Array.isArray(manifest.chapters) ? manifest.chapters : [];
    const c = chapters.find(function (x) { return x.id === chapterId; });
    if (!c) return { ok: false, error: "章节不存在：" + chapterId };
    c.title = title;
    await this.writeManifest(manifest);
    if (renameBody !== false) {
      try {
        const body = await this.readText(c.path);
        const nb = String(body).replace(/^##\s*第?\s*[0-9一二三四五六七八九十百千]+\s*[章节回]\s*[:：]?\s*[^\n]*/, "## " + title);
        if (nb !== body) await this.writeText(c.path, nb);
      } catch (err) {}
    }
    return { ok: true, title: title };
  }

  async renameVolume(volume, newVolume) {
    const manifest = await this.readManifest();
    const chapters = Array.isArray(manifest.chapters) ? manifest.chapters : [];
    const sv = String(newVolume).replace(/[\\/:*?"<>|]+/g, "-");
    const hits = chapters.filter(function (x) { return (x.volume || "") === volume; });
    if (!hits.length) return { ok: false, error: "分卷不存在或没有章节：" + volume };
    let moved = 0;
    for (const c of hits) {
      c.volume = newVolume;
      if (sv !== volume) {
        const oldRel = c.path;
        const bare = String(oldRel).split("/").pop();
        const newRel = "chapters/" + sv + "/" + bare;
        try {
          const body = await this.readText(oldRel);
          await this.writeText(newRel, body);
  ...
  async _gatherIndexDocs() {
    ...
    for (const root of roots) {
      const files = await this.listFiles(root);
      for (const rel of files) {
        if (rel.indexOf("INDEX.md") >= 0) continue; // 活目录不参与语义索引
        const fullRel = root + "/" + rel;
        let text;
  ...
  async vectorSearch(query, opts) {
    ...
    const backend = this.config.embedBackend || "deepseek";
    if (backend === "deepseek") {
      const chits = await this._conceptIdx().query(query, limit);
      if (!chits.length) return null;
      return chits.map(function (h) {
        return { rel: h.ref, score: h.score, snippet: h.text.slice(0, 200), matched: h.matched };
      });
    }
  ...
  async embedMissing(opts) {
    ...
    const backend = this.config.embedBackend || "deepseek";
    if (backend === "deepseek") {
      // DeepSeek 概念索引优先（零第三方依赖）
      const cs = await this._conceptIdx().buildDocs(missing.slice(0, cap), {});
      cs.chunks = 0;
      cs.remaining = 0;
      cs.reason = "llm-concepts";
      return cs;
    }
    const client = await resolveEmbedClient(this.config, this.credentials);
    if (!client) {
      // 无 embedding key：用 DeepSeek LLM 概念索引兜底（语义检索不依赖第三方）
      const stats = await this._conceptIdx().buildDocs(missing.slice(0, cap), {});
      const left = Math.max(0, idx.missingVectorDocs().length - stats.embedded);
      stats.remaining = left;
      stats.reason = stats.embedded ? "llm-concepts" : "no-key";
      return stats;
    }
    const stats = await embedDocs(idx, missing.slice(0, cap), client, {});
    if (stats.embedded === 0 && stats.failed > 0) {
      // 稠密嵌入失败（百炼额度耗尽等）：降级 DeepSeek LLM 概念索引
      const cs = await this._conceptIdx().buildDocs(missing.slice(0, cap), {});
      cs.chunks = 0;
      cs.remaining = 0; // 概念索引按内容 hash 增量，本轮传入已处理完毕

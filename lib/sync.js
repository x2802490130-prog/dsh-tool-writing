import { loadCharacters, saveCharacters } from "./characters.js";
  ...
  if (skip.indexOf("state") < 0) {
    try {
      const raw = await engine.generate({
        ...
        prompt: "【正文】\n" + joined + "\n\n请输出 JSON：{\"characters\":[{\"name\":\"姓名\",\"want\":\"此刻想要\",\"fear\":\"此刻害怕\",\"lost\":\"刚失去\",\"relation\":\"人际变化\"}（只输出本章出场且状态较之前有变化的人物；四项完整给出，没有的项给空串）],\"places\":[{\"name\":\"地点名\",\"note\":\"状态备注\"}],\"people\":[{\"name\":\"人名\",\"role\":\"身份\",\"aliases\":[\"别名\"],\"note\":\"备注\"}],\"objects\":[{\"name\":\"物证名\",\"location\":\"当前在何处/何人手中\",\"source\":\"来自哪里\"}],\"terms\":[{\"canonical\":\"规范称谓\",\"aliases\":[\"不规范的变体\"],\"note\":\"说明\"}]}。未涉及的字段输出空数组；拿不准的字段留空字符串。",
        json: true,
        signal: opts.signal
      });
      ...
      if (Array.isArray(patch.characters) && patch.characters.length) {
        ...
        await saveCharacters(engine, { characters: list });
      }
      let wsN = 0;

...
export async function runSync(engine, opts) {
  ...
  if (skip.indexOf("summarize") < 0) {
    try {
      ...
      // 活大纲：摘要同步沉淀到 outline/auto-synopsis.md（大纲随故事成长，不覆盖作者手写大纲）
      try {
        const orel = "outline/auto-synopsis.md";
        let osyn = "";
        try { osyn = await engine.readText(orel); } catch (err) { osyn = "# 章节细纲（自动沉淀，随故事成长）\n"; }
        const head = "## " + label + " · " + (targets[0] && targets[0].title ? targets[0].title : "") + "\n" + String(summary).trim();
        await engine.writeText(orel, (osyn.trim() + "\n\n" + head).trim() + "\n");
      } catch (err) {}
      // 台账同步最近摘要
      try {

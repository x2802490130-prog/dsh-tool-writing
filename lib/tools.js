...
async function gatherContext(engine, opts) {
  ...
  if (opts.lore !== false) {
    ...
    const cards = files.filter(function (f) { return systemFiles.indexOf(f) < 0 && f.indexOf("README") < 0 && !/^other\/[^/]*-profile\.md$/.test(f); });

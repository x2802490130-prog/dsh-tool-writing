...
import { readChoice, writeChoice, generateOptions, sleep } from "./choice.js";
...
system: "你是设定演化追踪师。对比人物卡/设定与最新正文，检测细节演变（性格/能力/关系/动机/心理/世界观）。",
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const choice = { state: "pending", id: "choice-" + Date.now().toString(36), fork: fork, context: args.context || "", options: opts, answer: null, answeredAt: null };
      await writeChoice(engine, choice);
      const lines = ["已推演 " + opts.length + " 个走向，推送到写作面板「抉择」小窗：", ""];
      for (const o of opts) {
      ...
      lines.push("");
      lines.push("请作者在面板小窗中选择或扩写（选择总是可以出乎意料地推进），然后用 novel_wait_choice 等待答案。");
      return lines.join("\n");
    }
  }));

  ctx.tools.register(defineTool({
    name: "novel_wait_choice",
    description: "写作交互：轮询等待作者在「抉择」小窗提交的选择与扩写（阻塞至作者作答或超时）。拿到答案后把它作为下一章生成的指令。",
    parameters: {
      timeoutMs: { type: "integer", description: "等待毫秒数，默认 300000（5 分钟），上限 30 分钟" }
    },
    output: TEXT_OUTPUT,
    async execute(args) {
      const timeoutMs = Math.min(30 * 60 * 1000, Math.max(5000, args.timeoutMs || 5 * 60 * 1000));
      const t0 = Date.now();
      let c = await readChoice(engine);
      if (!c) return "当前没有待抉择（请先用 novel_choice 发起）。";
      if (c.state === "answered" && c.answer) {
        const a = c.answer;
        return "作者已抉择：" + (a.selected ? "【" + a.selected + "】" : "【自定义走向】") + (a.expanded ? "\n作者扩写/修改：" + a.expanded : "") + "\n\n请将以上选择与扩写融入下一章生成指令。";
      }
      if (c.state === "cancelled") return "作者取消了本次抉择。";
      while (Date.now() - t0 < timeoutMs) {
        c = await readChoice(engine);
        if (c && c.state === "answered" && c.answer) {
          const a = c.answer;
          return "作者已抉择：" + (a.selected ? "【" + a.selected + "】" : "【自定义走向】") + (a.expanded ? "\n作者扩写/修改：" + a.expanded : "") + "\n\n请将以上选择与扩写融入下一章生成指令。";
        }
        if (c && c.state === "cancelled") return "作者取消了本次抉择。";
        await sleep(2000);
      }
      return "等待超时（作者未作答）。稍后可再次调用 novel_wait_choice。";
    }
  }));
;

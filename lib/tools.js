...
export { splitFrontMatter, renderCard, writeLoreCard, appendEvolutionToCard, gatherContext, chapterNumberFromId, normalizeHeading };
...
async function gatherContext(engine, opts) {
  ...
  // 技法指纹：叙事手法的示范（自我打断/内在叙事/配角剩余人格/物象分层/钩子变化/设定展示）
  try {
    const tech = await engine.readText("lore/other/technique-profile.md");
    if (tech && tech.trim()) {
      parts.push("");
      parts.push("【叙事技法示范（参考其手法写本章：节奏有打断、人物有内心、配角有剩余人格、物象分层揭开、钩子不重复）】");
      parts.push(tech);
    }
  } catch (e) {}
  return parts.join("\n");

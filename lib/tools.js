  ...
  for (const o of outcomes) {
    ...
    if (o.kind === "p") {
      ...
      for (const x of o.hits) {
        ...
        if (seen.has(key)) continue; seen.add(key);
        candidates.push({ source: "书库(向量)", rel: x.novelTitle + " · 片段" + x.chunkIdx, snippet: x.snippet, score: x.score });
      }
    }
  }
  try {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (target === "project" || target === "all") {
        ...
        lines.push("项目：" + (stats.reason ? stats.reason : ("新嵌 " + stats.embedded + " 篇 / " + stats.chunks + " 块" + (stats.failed ? "，失败 " + stats.failed : "") + (stats.remaining ? "，剩余 " + stats.remaining : "，已全部嵌入"))));
      }
      if (target === "library" || target === "all") {
        let stats;
        if (args.novelId) stats = await library.embedNovel(args.novelId);
        else stats = await library.embedAll();
        lines.push("书库：" + (stats.error ? stats.error : ("处理 " + stats.novels + " 本 / 新嵌 " + stats.embedded + " 章 / " + stats.chunks + " 块" + (stats.failed ? "，失败 " + stats.failed : ""))));
      }

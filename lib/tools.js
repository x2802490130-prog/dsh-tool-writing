  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (target === "project" || target === "all") {
        ...
        lines.push("项目：" + (stats.reason === "llm-concepts-fallback" ? ("概念索引新嵌 " + stats.embedded + " 篇（embedding 不可用，已自动降级 DeepSeek 语义索引" + (stats.remaining ? "，剩余 " + stats.remaining : "") + "）") : (stats.reason ? stats.reason : ("新嵌 " + stats.embedded + " 篇 / " + stats.chunks + " 块" + (stats.failed ? "，失败 " + stats.failed : "") + (stats.remaining ? "，剩余 " + stats.remaining : "，已全部嵌入")))));
      }

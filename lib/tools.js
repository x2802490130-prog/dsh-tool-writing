...
export function registerWritingTools(ctx, engine, library) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      await engine.writeText(rel, text);
      // 自动提取本章标题：正文标题行（## 第N章 X）→ instruction 中的《X》→ 空
      let title = "";
      const tm = String(text).match(/^##\s*第?\s*[0-9一二三四五六七八九十百千]+\s*[章节回]\s*[:：]?\s*([^\n]+)/) || String(text).match(/^##\s*([^\n]+)/);
      if (tm) title = tm[1].trim();
      if (!title) {
        const mi = String(args.instruction || "").match(/《([^》]+)》/);
        if (mi) title = mi[1].trim();
      }
      await recordChapter(engine, chapterId, title, countWords(text), rel, args.volume);
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (args.action === "chapter") {
        ...
        const c = chapters.find(function (x) { return x.id === cid; });
        if (!c) return "章节不存在：" + cid;
        c.title = nt;
        await engine.writeManifest(manifest);
        if (args.renameBody !== false) {
          try {
            const body = await engine.readText(c.path);
            const nb = String(body).replace(/^##\s*第?\s*[0-9一二三四五六七八九十百千]+\s*[章节回]\s*[:：]?\s*[^\n]*/, "## " + nt);
            if (nb !== body) await engine.writeText(c.path, nb);
          } catch (e) {}
        }
        return "章节已改名：" + cid + " → 《" + nt + "》";
      }
      if (args.action === "volume") {
        ...
        const sv = safeName(nv);
        const hits = chapters.filter(function (x) { return (x.volume || "") === ov; });
        if (!hits.length) return "分卷不存在或没有章节：" + ov;
        let moved = 0;
        for (const c of hits) {
          c.volume = nv;
          if (sv !== ov) {
            ...
            try {
              const body = await engine.readText(oldRel);
              await engine.writeText(newRel, body);
              await engine.deleteFile(oldRel);
              c.path = newRel;
              moved++;
            } catch (e) {}
          }
        ...
        await engine.writeManifest(manifest);
        await engine.rebuildCatalog();
        return "分卷已改名：「" + ov + "」→「" + nv + "」，迁移 " + moved + " 个章节文件。";
      }
      return "未知 action";
    }
  }));

...
export function registerWritingTools(ctx, engine, library) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const text = await engine.generate({
        system: "你是写长篇小说的。以人物视角写作：进入主要人物此刻的所知、所感与欲望，用场景、对话与行动呈现。白描克制：环境一笔带过，比喻少而准，让动作与对话推进故事。",
        prompt: charCtx + context + "\n\n【本章】" + (args.title ? "标题参考：" + args.title + "。" : "") + (args.instruction ? "作者提示：" + args.instruction + "。" : "从人物当下的处境自然推演。") + " 写到这场戏自然收束（约 " + target + " 字上下，不必凑数）。第一行输出章节标题（2-8字，提炼本章核心），格式 ## 第N章 标题。",
        signal: exec && exec.signal
      });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const text = await engine.generate({
        system: "你是写长篇小说的。以人物视角写作：进入主要人物此刻的所知、所感与欲望，用场景、对话与行动呈现。白描克制：环境一笔带过，比喻少而准，让动作与对话推进故事。",
        prompt: (charCtx ? charCtx + "\n" : "") + context + "\n\n【续写】从上一章结尾处，以人物自己的选择推演接下来发生的事" + (args.instruction ? "（作者提示：可以朝这个方向：" + args.instruction + "）" : "") + "。写到这场戏自然收束（约 " + target + " 字上下，不必凑数）。第一行输出章节标题（2-8字，提炼本章核心），格式 ## 第N章 标题。",
        signal: exec && exec.signal
      });
      const vol = args.volume ? safeName(args.volume) : "";

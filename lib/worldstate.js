...
export function worldStateContext(data) {
  ...
  lines.push.apply(lines, renderList((data.objects || []).slice(-10), "- 物证：", function (o) { return o.name + (o.location ? "→" + o.location : "") + (o.source ? "（来自：" + o.source + "）" : ""); }));
...
function upsertByName(arr, patchItems) {
  ...
  for (const it of (Array.isArray(patchItems) ? patchItems : [])) {
    ...
    for (const k of ["note", "location", "source", "role", "chapter"]) {
